  ...
  head: () => ({
    meta: [
      { title: "Cultural Heritage of India — Bharat Darshan" },
      {
        name: "description",
        content:
          "Temples, Mughal gardens, classical dance, textiles, festivals and cuisines — the living heritage of India.",
      },
      ...
      <section className="mx-auto max-w-6xl px-5 py-16">
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {heritage.map((h) => (
            <article
              key={h.title}
              className="group rounded-3xl border border-border bg-card p-7 transition-all duration-300 hover:-translate-y-1 hover:shadow-lift"
            >
              ...
              <p className="mt-4 text-sm text-accent">{h.place}</p>
            </article>
          ))}
        </div>
      </section>
